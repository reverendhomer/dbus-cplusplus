...
void handler1(const void *data, void *buffer, unsigned int nbyte)
{
  ...
  std::cout << "buffer1: " << str << ", size: " << nbyte << std::endl;
  for (int i = 0; i < 30 && spin; ++i)
    std::cout << "call1: " << g_client->Hello(str) << std::endl;
}

void handler2(const void *data, void *buffer, unsigned int nbyte)
{
  auto str = static_cast<char *>(buffer);
  std::cout << "buffer2: " << str << ", size: " << nbyte << std::endl;
  for (int i = 0; i < 30 && spin; ++i)
    std::cout << "call2: " << g_client->Hello(str) << std::endl;
}

void handler3(const void *data, void *buffer, unsigned int nbyte)
{
  auto str = static_cast<char *>(buffer);
  std::cout << "buffer3: " << str << ", size: " << nbyte << std::endl;
  for (int i = 0; i < 30 && spin; ++i)
    std::cout << "call3: " << g_client->Hello(str) << std::endl;
}
